...
export default async function (request, response) {
    ...
    // OpenAI API anahtarını Vercel Ortam Değişkenlerinden (Environment Variables) al.
    // Bu, anahtarın kodda açıkça görünmesini engeller ve güvenliği artırır.
    const openai = new OpenAI({
        apiKey: process.env.OPENAI_API_KEY,
    });

    // Yapay zekaya verilecek detaylı talimat (prompt) metni.
    const promptText = `
    ...
    try {
        ...
        // Gelen yanıtın olumsuz kelimeler içerip içermediğini kontrol et.
        ...
        if (negativeKeywords.some(keyword => falYorumu.toLowerCase().includes(keyword))) {
             // Eğer olumsuz bir kelime bulunursa, varsayılan bir yorum gönder.
             // Bu, kullanıcının hiçbir zaman olumsuz bir mesaj almasını engeller.
...
3. Sağlık: Finandaki ağaç dalları figürü, köklerinize dönmeniz ve doğayla iç içe olmanız gerektiğini gösteriyor. Bu, ruhsal ve fiziksel sağlığınız için çok faydalı olacaktır.
...
5. Genel Yorum: Fincanınızın genel görünümü, içsel bir dönüşümden geçtiğinizi gösteriyor. Geçmişin yüklerinden kurtulup, daha parlak bir geleceğe doğru emin adımlarla ilerliyorsunuz.
        ...
        // Her şey yolundaysa, OpenAI'den gelen yorumu doğrudan gönder.
